...
import com.openvp.shortVideo.AVBaseJni;
import com.openvp.shortVideo.FFMpegUtils;
import com.openvp.shortVideo.model.MediaObject;


/**
 * 视频录制：边录制边底层处理视频（旋转和裁剪）
 */
public class MediaRecorderNative extends MediaRecorderBase implements MediaRecorder.OnErrorListener, AVBaseJni.FFmpegStateListener {

    public MediaRecorderNative() {
        AVBaseJni.registFFmpegStateListener(this);
    }
    ...
    public MediaObject.MediaPart startRecord() {
        ...
        if (mCameraId == Camera.CameraInfo.CAMERA_FACING_BACK) {
            vCustomFormat=AVBaseJni.ROTATE_90_CROP_LT;
        } else {
            vCustomFormat=AVBaseJni.ROTATE_270_CROP_LT_MIRROR_LR;
        }

        AVBaseJni.InitEncoder( mMediaObject.getOutputDirectory(), mMediaObject.getBaseName(),vCustomFormat, mSupportedPreviewWidth, SMALL_VIDEO_HEIGHT, SMALL_VIDEO_WIDTH, SMALL_VIDEO_HEIGHT, mFrameRate, mVideoBitrate);
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        if (mRecording) {
            ...
            AVBaseJni.encodeFrame2H264(data, length);
...
//        UtilityAdapter.RenderOutputSettings(SMALL_VIDEO_WIDTH, SMALL_VIDEO_HEIGHT, mFrameRate, UtilityAdapter.OUTPUTFORMAT_YUV | UtilityAdapter.OUTPUTFORMAT_MASK_MP4/*| UtilityAdapter.OUTPUTFORMAT_MASK_HARDWARE_ACC*/);
    ...
    public void receiveAudioData(byte[] sampleBuffer, int len) {
        if (mRecording && len > 0) {
            AVBaseJni.encodeFrame2AAC(sampleBuffer,len);
        }
    }
    ...
    public void allRecordEnd() {
        ...
        Log.e("allRecordEnd","need save to local");
    ...
    public void activityStop(){
        AVBaseJni.unRegistFFmpegStateListener(this);
    }
}
